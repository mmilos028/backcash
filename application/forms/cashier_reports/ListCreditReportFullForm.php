...
class ListCreditReportFullForm extends Zend_Form {
	public function __construct($options = null) {
		parent::__construct($options);
		$baseUrl = Zend_Controller_Front::getInstance()->getBaseUrl();
		$this->setName('list-credit-report');
		$this->setMethod('post');
		$lang = Zend_Registry::get("lang");
		$this->setAction($baseUrl . '/'. $lang . '/cashier-reports/credit-report');
        ...
        $game_types = new Zend_Form_Element_Select('GAME_TYPE');
		$game_types->setOrder($i);
		$game_types->setLabel($translate->_("Flash / HTML5"));
		$game_types->addMultiOption(ALL, $translate->_("Select All"));
        $game_types->addMultiOption("Flash", $translate->_("Flash"));
        $game_types->addMultiOption("HTML5", $translate->_("HTML5"));
		$game_types->setRequired(true);
        $game_types->setDecorators($selectDecorator);
        $game_types->setAttribs(
            array(
                'tabindex'=>$i,
		...
		$previous_page->setAttrib('tabindex', $i);
		...
		$min_report->setAttrib('tabindex', $i);
		...
		$this->setDecorators(array(array('ViewScript', array('viewScript'=>'viewScripts/cashier_reports/ListCreditReportFullViewScript.phtml'))));
		$this->addElements(array($game_types, $affiliates, $startdate, $enddate, $pageNo, $limitItems,$submit, $level_direction, $currency_list, $affiliate_number,
            $previous_page, $min_report
        ));
	}
}