...
class TransferCreditFromCashierToPlayerTerminalCompletedForm extends Zend_Form {
	public function __construct($options = null) {
		parent::__construct($options);
		$baseUrl = Zend_Controller_Front::getInstance()->getBaseUrl();
		$this->setName('transfer-credit-from-cashier-to-player-terminal-completed');
		$this->setMethod('post');
		...
		$affiliateName->setLabel($translate->_("Player_Aff"))->addFilter("StripTags")->addFilter("StringTrim")
            ->setAttrib("disabled", true)
            ->setAttrib("class", "form-control bold-text margin-1")
            ->setE;
		...
		$this->setDecorators(array(array('ViewScript', array('viewScript'=>'viewScripts/financial_transaction/for_touchscreen/CreditTransferFromCashierToPlayerTerminalCompletedViewScript.phtml'))));
