...
class UpdatePlayerForm extends Zend_Form {
	public function __construct($options = null) {
        ...
        $currency->setValue($translate->_("SelectCurrency"));
        $currency->setDecorators($textDecorator);
        $currency->setValue($_SESSION["auth_space"]["session"]["currency"]);
        $currencyHidden->setValue($_SESSION["auth_space"]["session"]["currency"]);
        $i++;
		...
		$subject_path = new Zend_Form_Element_Text("PLAYER_LOCATION");
		$subject_path->setDecorators($textDecorator);
        $subject_path->setOrder($i);
        $subject_path->setAttribs(
            array(
                'disabled'=>'',
                'tabindex'=>$i,
                'class'=> 'form-control margin-1'
		...
		$this->setDecorators(array(array('ViewScript', array('viewScript'=>'viewScripts/players/UpdatePlayerViewScript.phtml'))));
		$this->addElements(array($player_name, $email, $first_name, $last_name, $birthday, $zip, $phone, $city,
            $address, $country, $currency, $currencyHidden, $banned, $submit, $cancel, $type, $csrf, $subject_path));
	}
}